...
export const api = {
  ...
  importKeys: (text, name, dry = false) => request('/import/keys', { method: 'POST', body: JSON.stringify({ text, name, dry }) }),
  ...
  exportKeys: (name) => request(`/export/keys?name=${name}`),
  ...
  bannedCheck: (force = false) => request('/accounts/banned-check', { method: 'POST', body: JSON.stringify({ force }) }),
  githubSummary: () => request('/github/summary'),
