...
| | 密钥导出 | opencode / freebuff 密钥按「账号-密钥」格式一键导出（弹窗预览/复制/下载），与导入格式一致可往返 |
| | 凭证标记 | 账号列表显示 opencode / freebuff 凭证徽章，有则标注、无则不显示 |
| | 凭证筛选 | 按「有 opencode / 有 freebuff / 两者都有 / 两者都没有」筛选账号 |
...
>
> 💡 **反代后限速/审计取真实 IP**：systemd 服务里加 `Environment=TRUST_PROXY=1`，登录限速锁定与审计日志将取 `X-Forwarded-For` 首跳 IP（仅受控反代下开启，否则攻击者可伪造 XFF 绕过限速）。
